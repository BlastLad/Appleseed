...
public class SpotlightFieldOfView : MonoBehaviour
{
    ...
    void FindVisibleTargets()
    {
        ...
        int blockMask = LayerMask.GetMask("Objects", "Walls", "SmallWalls", "Pitfalls"); //Maybe Pitfalls, and SmallWalls or delete them...
        ...
        for (int i = 0; i < targetsInViewRadius.Length; i++)
        {
            ...
            if (Vector3.Angle(-transform.up, directionToTarget) < viewAngle / 2)
            {
                ...
                if (!Physics2D.Raycast(transform.position, directionToTarget, distanceToTarget, blockMask))
                {
                    ...
                    bool vulnState = spotLight.GetComponent<SpotlightEnemyController>().GetVulnState();
                       
                    if (vulnState == false)
                    {
                        spotLight.GetComponent<SpotlightEnemyController>().FindTarget(target.gameObject);
                    }
                }
            }
        }
    ...
    ViewCastInfo ViewCast(float globalAngle)
    {
        ...
        int blockMask = LayerMask.GetMask("Objects", "Walls", "SmallWalls");
